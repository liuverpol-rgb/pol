# Zur Mühle Reinbek — Hannemanns Mühle

Datos que faltan para dejar su web terminada. Pregúntalos en la llamada o
cuando conteste a la carta, y escríbelos en `clientes/zur-muehle.json`.

**9 huecos.** Después: `node webs-locales/generar.mjs clientes/zur-muehle.json`
...
> Was soll bei Google unter dem Namen stehen?
  ...
  - `seo.descripcion` — Zur Mühle Reinbek — Hannemanns Mühle in Reinbek: Restaurant. [bestätigen]
...
> Welche Leistungen bieten Sie an, und was kosten sie?
  ...
  - `secciones[0].entradilla` — Zeiten und Preise [bestätigen].
  - `secciones[0].items[0].precio` — [bestätigen] €
  - `secciones[0].items[1].descripcion` — [bestätigen]
  - `secciones[0].items[1].precio` — [bestätigen] €
  - `secciones[1].items[0].descripcion` — [bestätigen] — bis wie viele Personen?
...
| Dato | Valor |
| --- | --- |
| Nombre | Zur Mühle Reinbek — Hannemanns Mühle |
| Tipo | Restaurant |
| Ciudad | Reinbek |
| Teléfono | +49 40 72810538 |
| Dirección | Mühlenweg 1, 21465 Reinbek |
